  ...
  return (
    <DragDropContext onDragEnd={handleDragEnd}>
      <div className="flex gap-4 items-start">
        ...
        {/* Officers Panel */}
        <div className="w-36 shrink-0 bg-[#1a2744] rounded-xl border border-[rgba(212,168,67,0.1)] p-3 sticky top-20">
          <p className="text-[10px] uppercase tracking-widest text-[#d4a843] font-bold mb-3">Officers</p>
          <Droppable droppableId="officers-panel" isDropDisabled={true}>
            {(provided) => (
                  ...
                  <Draggable key={u.email} draggableId={u.email} index={i}>
                    {(provided, snapshot) => (
                      <div
                        ...
                        className={`flex items-center gap-1.5 bg-[#0a1128] rounded px-2 py-1.5 text-xs text-white cursor-grab active:cursor-grabbing transition-all ${snapshot.isDragging ? "opacity-70 scale-105 shadow-lg border border-[#d4a843]/40" : "border border-transparent"}`}
                      >
                            ...
                            <Droppable key={droppableId} droppableId={droppableId}>
                              {(provided, snapshot) => (
                                <div
                                  ...
                                  className={`flex items-start gap-2 rounded-lg px-2 py-1.5 min-h-[36px] border transition-all ${snapshot.isDraggingOver ? "bg-[#d4a843]/10 border-[#d4a843]/40" : "bg-[#0a1128]/60 border-transparent"}`}
                                >
                                  <span className="text-[10px] text-slate-500 shrink-0 w-24 pt-0.5 truncate">{pos.name}</span>
                                  <div className="flex flex-wrap gap-1 flex-1">
                                    {cellAssignments.map(a => (
                                      ...
                                      >
                                        ...
                                        <button onClick={() => handleRemove(a.id)} className="opacity-0 group-hover:opacity-100 text-red-400 hover:text-red-300 transition-opacity ml-0.5">

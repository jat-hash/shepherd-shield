                   ...
                   <Draggable key={u.email} draggableId={u.email} index={i}>
                     {(provided, snapshot) => (
                       <div
                         ...
                         className={`flex items-center gap-2 bg-[#0a1128] rounded px-2 py-1 text-xs text-white cursor-grab active:cursor-grabbing transition-all min-w-0 ${snapshot.isDragging ? "opacity-70 scale-105 shadow-lg border border-[#d4a843]/40" : "border border-[rgba(212,168,67,0.1)]"}`}
                       >
                         <GripVertical className="w-3 h-3 text-slate-600 shrink-0" />
                         <span className="text-[10px] whitespace-nowrap ml-auto truncate">{u.data?.display_name || u.display_name || u.full_name || u.email.split("@")[0]}</span>

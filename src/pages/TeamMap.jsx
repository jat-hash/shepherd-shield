              ...
              .map((a) => (
                <div key={a.id} className="px-3 py-2 border-b border-[rgba(255,255,255,0.04)] hover:bg-white/5 flex items-center gap-2">
                  {(() => {
                    ...
                    const photoUrl = userWithEmail?.profile_photo || userWithEmail?.data?.profile_photo;
                    if (photoUrl) {
                      return <img src={photoUrl} alt={a.assigned_to_name} className="w-6 h-6 rounded-full object-cover border border-[#d4a843]" />;
                    }
                    return <div className="w-6 h-6 rounded-full bg-gradient-to-br from-[#d4a843] to-[#b8902a] flex items-center justify-center text-[#0a1128] text-[9px] font-bold border border-[#d4a843]">{(a.assigned_to_name || '?').charAt(0).toUpperCase()}</div>;
                  })()}

  ...
  return (
    <div className="min-h-screen bg-[#0a1128] text-white p-4 lg:pl-60">
      ...
      <Dialog open={profileDialogOpen} onOpenChange={setProfileDialogOpen}>
        <DialogContent className="bg-[#141f3d] border-[rgba(212,168,67,0.15)] text-white max-w-md">
          <DialogHeader>
          ...
          {profileUser && (
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-4">
                <div className="w-16 h-16 rounded-full bg-gradient-to-br from-[#d4a843] to-[#b8902a] flex items-center justify-center text-[#0a1128] text-2xl font-bold flex-shrink-0">
                  {(profileUser.display_name || profileUser.full_name)?.charAt(0) || "U"}
                </div>
                <div>
                  <h3 className="text-lg font-bold text-white">{profileUser.display_name || profileUser.full_name || "Team Member"}</h3>
                  <div className="flex flex-wrap gap-1 mt-1">
                    {profileUser.role && (
                      <span className={`px-2 py-0.5 rounded-full text-[10px] font-semibold border ${profileUser.role === 'admin' ? 'bg-[#d4a843]/20 text-[#d4a843] border-[#d4a843]/30' : 'bg-slate-700 text-slate-300 border-slate-600'}`}>
                        {profileUser.role === 'admin' ? '⭐ Admin' : profileUser.role}
                      </span>
                    )}
                    {profileUser.command_position && (
                      <span className="px-2 py-0.5 rounded-full text-[10px] font-semibold bg-purple-900/50 text-purple-200 border border-purple-700/50">
                        {profileUser.command_position}
                      </span>
                    )}
                  </div>
              ...
              <div className="bg-[#1a2744] rounded-lg p-3 space-y-2">
                ...
                {profileUser.created_date && (
                  <div className="flex items-center gap-2 text-sm text-slate-300">
                    ...
                    <span>Joined {new Date(profileUser.created_date).toLocaleDateString()}</span>
                  </div>
                )}
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-[#1a2744] rounded-lg p-3 text-center">
                  {profileStats ? (
                    <p className="text-2xl font-bold text-blue-400">{profileStats.assignments}</p>
                  ) : (
                    <div className="w-6 h-6 border-2 border-blue-400 border-t-transparent rounded-full animate-spin mx-auto" />
                  )}
                  <p className="text-[10px] text-slate-500 uppercase tracking-wider mt-1">Assignments</p>
                </div>
                <div className="bg-[#1a2744] rounded-lg p-3 text-center">
                  {profileStats ? (
                    <p className="text-2xl font-bold text-orange-400">{profileStats.incidents}</p>
                  ) : (
                    <div className="w-6 h-6 border-2 border-orange-400 border-t-transparent rounded-full animate-spin mx-auto" />
                  )}
                  <p className="text-[10px] text-slate-500 uppercase tracking-wider mt-1">Incidents</p>
                </div>
              </div>

              {/* Notification Prefs */}
              <div className="bg-[#1a2744] rounded-lg p-3">
                <p className="text-xs text-slate-500 uppercase tracking-wider mb-2">Notification Preferences</p>
                <div className="space-y-1.5 text-xs text-slate-300">
                  <div className="flex items-center gap-2">
                    {profileUser.notifications_email !== false ? <Bell className="w-3.5 h-3.5 text-green-400" /> : <BellOff className="w-3.5 h-3.5 text-slate-500" />}
                    <span>Email {profileUser.notifications_email !== false ? 'on' : 'off'}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    {profileUser.notifications_sms ? <Bell className="w-3.5 h-3.5 text-green-400" /> : <BellOff className="w-3.5 h-3.5 text-slate-500" />}
                    <span>SMS {profileUser.notifications_sms ? 'on' : 'off'}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    {profileUser.notifications_in_app !== false ? <Bell className="w-3.5 h-3.5 text-green-400" /> : <BellOff className="w-3.5 h-3.5 text-slate-500" />}
                    <span>In-app {profileUser.notifications_in_app !== false ? 'on' : 'off'}</span>
                  </div>
                </div>
              </div>
            </div>

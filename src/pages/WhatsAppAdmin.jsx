  ...
  return (
    <div className="max-w-2xl mx-auto px-3 py-4 lg:px-4 lg:py-6 lg:ml-60 space-y-5">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-lg font-bold text-white">WhatsApp Integration</h1>
          <p className="text-xs text-slate-400">Manage WhatsApp bot, broadcasts & check-ins</p>
        </div>
      </div>
      ...
      <div className="bg-[#1a2744] rounded-xl border border-[rgba(212,168,67,0.1)] p-5 space-y-4">
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between gap-3 bg-[#0a1128] rounded-lg p-3">
            <div>
              <p className="text-sm text-white font-medium">Safety Check-In Blast</p>
              <p className="text-xs text-slate-400 mt-0.5">Send WhatsApp check-in request to all members with a phone number</p>
            </div>
            <WhatsAppSafetyCheckinButton alert={activeAlerts[0] || null} className="text-xs h-8 px-3 flex-shrink-0" />
          </div>
      ...
      {/* Check-In Log */}
      <div className="bg-[#1a2744] rounded-xl border border-[rgba(212,168,67,0.1)] p-5 space-y-3">
        <div className="flex items-center gap-2">
          <Users className="w-4 h-4 text-[#d4a843]" />
          <h3 className="text-sm font-bold text-white">Recent Check-Ins</h3>
          <span className="ml-auto text-[10px] text-slate-500">{checkins.length} total</span>
        </div>

        {checkins.length === 0 ? (
          <p className="text-slate-500 text-sm text-center py-4">No check-ins yet. They'll appear here when users respond via WhatsApp.</p>
        ) : (
          <div className="space-y-2 max-h-80 overflow-y-auto">
            {checkins.map(c => (
              <div key={c.id} className="flex items-center gap-3 bg-[#0a1128] rounded-lg p-3">
                <div className={`text-[10px] px-2 py-0.5 rounded-full font-semibold uppercase ${statusColors[c.status]}`}>
                  {c.status === 'need_help' ? 'HELP' : c.status}
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-white text-xs font-medium truncate">{c.user_name}</p>
                  <p className="text-slate-500 text-[10px] truncate">{c.user_email}</p>
                </div>
                {c.latitude && c.longitude && (
                  <a
                    ...
                    className="text-[10px] text-[#25D366] hover:underline flex-shrink-0"
                  >
                    📍 Map
                  </a>
                )}

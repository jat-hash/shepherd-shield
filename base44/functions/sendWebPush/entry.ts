...
// Web Crypto spec returns DER, but be defensive: if the runtime already
// returns raw r||s (64 bytes), pass it through unchanged.
...
async function encryptPayload(payloadObj, p256dhB64, authB64) {
  const userPub = b64urlDecode(p256dhB64);    // 65-byte uncompressed P-256 point
  ...
  const ecdhPubBytes = new Uint8Array(await crypto.subtle.exportKey('raw', ecdhKeys.publicKey)); // 65 bytes

  // Shared secret via ECDH with the subscription's p256dh public key
  ...
  // Header: salt(16) || rs(4, big-endian 4096) || keylen(1, 65) || ephemeral_pub(65)
...
Deno.serve(async (req) => {
  try {
    ...
    // Accept either an automation payload ({ event, data }) or a direct call
    // ({ recipient_email, title, body, notification_type, click_url, dm_channel }).
    ...
    // Payload shape matches what web-push-sw.js parsePayload() expects
    // (reads data.title / data.body / data.notification_type / ...).
    const payloadObj = {
      data: {
        title: String(title),
